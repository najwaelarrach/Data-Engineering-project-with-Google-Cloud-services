...
if __name__ == '__main__':
    ...
    TRIP_DATA_PATH = os.environ.get(
        'TRIP_DATA_PATH', 'gs://gcp-pyspark-airflow/trip_data_test.csv')
    FARE_DATA_PATH = os.environ.get(
        'FARE_DATA_PATH', 'gs://gcp-pyspark-airflow/fare_data_test.csv')
    BUCKET_NAME = os.environ.get(
        'BUCKET_NAME', 'new-york-taxi-tutorial-project-temporary-bucket')
    ...
    trip_data = spark.read.option("header", True).option(
        "inferSchema", True).csv(TRIP_DATA_PATH)
    fare_data = spark.read.option("header", True).option(
        "inferSchema", True).csv(FARE_DATA_PATH)
    ...
    for i in range(len(old_column_names)):
        trip_data = trip_data.withColumnRenamed(
    ...
    for i in range(len(old_column_names)):
        fare_data = fare_data.withColumnRenamed(
            old_column_names[i], new_column_names[i])
